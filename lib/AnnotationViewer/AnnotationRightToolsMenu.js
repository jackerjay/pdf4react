	...
	_createClass(AnnotationRightToolsMenu, [{
		...
		value: function handleSaveButtonClick() {
			...
			if (annotations.length > 0) {
				saveAnnotations = annotations.map(function (annotation) {
					return Object.assign({}, _extends({}, annotation, {
						text: annotationDivs[annotation.textId].text
					}));
				});

				var result = _superagent2.default.post(this.props.action).send(saveAnnotations).set('Accept', 'application/json').end(function (err, res) {
					if (err || !res.ok) {
						reject(res.body || err);
					} else {
						resolve(res.body);
		...
		value: function getSuccess(result) {
			...
			this.setState({
				message: _react2.default.createElement(_AnnotationMessage2.default, { message: result.body })
			});
